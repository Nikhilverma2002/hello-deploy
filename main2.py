# This code works far better. Humans >>>> A.I.
...
# # Branch + slots config (merged inside main.py)
# branches_config = {
#     "171": {   # Branch ID as string
#         "name": "Koramangala",
#         "slots": {
...
# days_to_fetch = 1   # Increase if needed
...
#                         print(f"Fetched for {branch_name} ({branch_id}) - {date_str} {time_str}")
#                     else:
#                         print(f"No slots for {branch_name} ({branch_id}) - {date_str} {time_str}")
#                 else:
#                     print(f"Error {response.status_code} for {branch_name} ({branch_id}) - {date_str} {time_str}")
#             except Exception as e:
#                 print(f"Exception for {branch_name} ({branch_id}) - {date_str} {time_str}: {e}")
...
#  ======================= Updating the above code with logic monitoring =============================================
import requests
import json
import pandas as pd
from datetime import datetime, timedelta
...
# Output path
output_path = "output/buffet_slots.xlsx"
...
def monitor(interval=300, days_to_fetch=1):  # default: every 5 minutes
    ...
    while True:
        ...
        if not df.empty:
            if last_df is None or not df.equals(last_df):
                ...
                print(f"[UPDATED] File overwritten at {output_path}")
                last_df = df.copy()
            else:
                print("[NO CHANGE] Data same as last check")
        else:
            print("[WARNING] No valid slot data collected")
...
if __name__ == "__main__":
    ...
    try:
        ...
        days_to_fetch = int(input("Enter number of days to fetch (default 1): ") or "1")
    except ValueError:
        print("Invalid input. Using defaults (300 sec, 1 day).")
        interval = 300
        days_to_fetch = 1

    monitor(interval=interval, days_to_fetch=days_to_fetch)
